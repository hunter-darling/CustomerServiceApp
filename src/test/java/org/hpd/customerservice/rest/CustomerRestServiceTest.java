...
class CustomerRestServiceTest {
    ...
    void testGetCustomers_noCity_success() throws SQLException {
        ...
        ResponseEntity<List<CustomerEntity>> customersResponseEntities = new ResponseEntity<>(List.of(new CustomerEntity()), null, 200);
    ...
    void testCreateCustomer_failure_409() {
        CreateCustomerRequest request = new CreateCustomerRequest();
        DataIntegrityViolationException dataIntegrityViolationException = new DataIntegrityViolationException("", new SQLException("UNIQUE constraint violated"));
        when(customerManager.addNewCustomer(request)).thenThrow(dataIntegrityViolationException);
